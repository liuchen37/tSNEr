...
The package tSNEr successfully captures the well-characterised developmental trajectory of T cells in thymic tissue, revealing distinct populations based on CD3, CD4, and CD8 expression patterns (\label{fig:3}). The analysis clearly delineates four major populations: CD4+CD8+ double positive (DP) cells forming the largest cluster, CD4+CD8- and CD4-CD8+ single positive mature T cells in separate regions, and CD4-CD8- double negative (DN) precursors. The CD3+ population (red arrow, panel A) are predominantly CD4 (blue arrow) or CD8 (orange arrow) single positives. 
